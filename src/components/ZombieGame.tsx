...
export default function ZombieGame() {
  ...
  useEffect(() => {
    ...
    const gameLoop = (timestamp: number) => {
      ...
      render(ctx, gameStateRef.current, CANVAS_WIDTH, CANVAS_HEIGHT);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-background gap-4 p-4">
      <h1 className="pixel-text text-primary text-lg md:text-2xl tracking-wider">
        🧟 ZOMBIE SLAYER 🧟
      </h1>
      ...
      <div className="relative pixel-border">
        ...
        {!gameStarted && !gameOver && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-background/90">
            <h2 className="pixel-text text-primary text-xl mb-8 animate-pulse">
              ZOMBIE SLAYER
            </h2>
            <div className="flex flex-col gap-3 text-muted-foreground pixel-text text-[8px] mb-8">
              <p>← → ou A/D: Mover</p>
            ...
            <button
              onClick={startGame}
              className="pixel-text text-accent bg-muted px-8 py-4 pixel-border hover:bg-primary hover:text-primary-foreground transition-colors text-sm"
            >
              ▶ INICIAR
            </button>
          </div>
        ...
        {gameOver && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-background/90">
            <h2 className="pixel-text text-destructive text-xl mb-4">
              GAME OVER
            </h2>
            <div className="flex flex-col gap-2 pixel-text text-[10px] text-foreground mb-6">
      ...
      <div className="flex gap-6 pixel-text text-[8px] text-muted-foreground">
        <span>← → MOVER</span>
        <span>↑ PULAR</span>
        <span>Z ATIRAR</span>
      </div>
